...
def conn_uri_data_to_db_sql_config():
    connection_uri = sa.engine.URL.create(
        "mssql+aioodbc", #"postgresql+asyncpg" for postgres, install asyncpg
        username=settings.DATABASE_USERNAME,\
        password=settings.DATABASE_PASSWORD,
        host=settings.DATABASE_HOST,
        database=settings.DATABASE_NAME,
        query=dict(driver='ODBC Driver 17 for SQL Server'), #remove this additional query for postgres
    )
